...
## AI Hub 안내서 직접 다운로드가 거부됨

### 증상

AI Hub 데이터 페이지의 안내서 주소를 HTTP 요청으로 바로 호출하자 파일 대신 다음 스크립트가 반환됐습니다.

```html
<script>alert('정상적인 접근이 아닙니다.');window.close();</script>
```

### 원인

안내서 주소만 직접 호출하면 AI Hub가 정상적인 페이지 경유 요청으로 처리하지 않았습니다. 데이터 상세 페이지와 다운로드 링크 자체는 정상적으로 열렸습니다.

### 해결

AI Hub 공식 데이터 상세 페이지에서 `어노테이션 포맷 및 데이터 구조`를 펼친 뒤 데이터 설명서와 구축활용가이드를 내려받았습니다. 저장소에는 안내서 원본을 추가하지 않고, 확인한 코드 정의와 적용 범위만 문서화했습니다.

### 검증

구축활용가이드 v1.4의 전자금융공동망 코드표에서 거래시간대 8개, 매체구분 7개, 자금구분 5개의 범위와 의미를 확인했습니다. 현재 학습 파일에 없는 자금구분 `11`은 모델 입력에서 계속 거부하도록 [이체 판별 입력 계약](bank-input-contract.md)에 구분해 기록했습니다.

## 활동일 추이 집계가 컴파일되지 않음

### 증상

날짜별 집계를 정렬한 뒤 최근 14개 활동일을 고르는 코드에서 Kotlin 컴파일이 실패했습니다.

```text
Unresolved reference 'takeLast'.
```

### 원인

`toSortedMap().entries`는 Set 형태이며, 현재 Kotlin 표준 라이브러리는 이 값에 `takeLast`를 바로 제공하지 않습니다. 뒤따른 구조 분해와 행 집계 오류는 첫 타입 추론 실패로 함께 발생했습니다.

### 해결

정렬된 항목을 `toList()`로 변환한 뒤 `takeLast(14)`를 적용했습니다. 날짜 정렬 순서는 유지됩니다.

### 검증

서로 다른 UTC 날짜의 판별 기록 30건을 저장하고, 날짜별 판별 수·경보 수·경보 비율·평균 위험 점수를 확인하는 테스트를 추가했습니다. Kotlin 테스트 7개가 모두 통과했습니다.

## 시스템 Python으로 테스트 의존성을 찾지 못함

### 증상

시스템 Python 3.14로 테스트를 실행하자 두 테스트 모듈이 로딩 단계에서 종료됐습니다.
...
프로젝트 의존성은 저장소의 `.venv`에 설치돼 있으며 시스템 Python 환경에는 `numpy`가 없었습니다. 코드나 테스트 로직의 실패는 아니었습니다.

### 해결

테스트 명령의 Python 실행 파일을 `.venv\Scripts\python.exe`로 바꿨습니다.

### 검증

프로젝트 가상환경에서 Python 테스트 7개를 다시 실행해 모두 통과했습니다.
