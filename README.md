...
대시보드는 모델 서비스와 검증 보고서의 버전을 비교합니다. 서로 다르거나 모델 서비스가 꺼져 있으면 거래 판별 버튼을 비활성화하고 검증 지표와 저장된 이력은 계속 표시합니다.
...
화면에서 카드·이체 판별부터 이력과 모니터링까지 확인하는 순서는 [시연 가이드](docs/demo-guide.md)를 참고하세요.
