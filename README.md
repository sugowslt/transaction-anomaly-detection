...
모니터링은 최근 판별 최대 1,000건을 학습 표본 382,040건의 집계 분포와 비교합니다. 거래금액은 Population Stability Index(PSI), 시간대·자금구분·매체구분은 Total Variation Distance(TVD)를 사용합니다. 30건 미만에서는 변화 상태를 판정하지 않습니다. PSI는 0.10·0.25, TVD는 0.10·0.20을 관찰·변화 큼의 경계로 사용하지만, 운영 판단을 돕기 위한 기준값일 뿐 모델 성능이나 실제 이상거래 증가를 뜻하지 않습니다. 원본 학습 행은 포함하지 않고 구간별 비율만 `reports/bank_baseline.json`에 저장합니다.
...
실제 거래 승인 시점에 각 입력 필드와 코드 정의가 제공되는지 확인한 뒤 운영용 입력 계약을 정할 예정입니다. 모니터링은 현재 최근 1,000건을 한 구간으로 비교하므로, 판별 데이터가 쌓이면 기간별 추이와 기준 분포 변경 이력을 추가할 계획입니다.
